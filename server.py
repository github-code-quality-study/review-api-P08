...
class ReviewAnalyzerServer:
    ...
    def __call__(self, environ: dict[str, Any], start_response: Callable[..., Any]) -> bytes:
        ...
        if environ["REQUEST_METHOD"] == "POST":
            ...
            if request_params.get('Location') is None:
                start_response("400 Bad Request", [("Content-Type", "text/plain")])
                return [b"Location is required"]
            if request_params.get('ReviewBody') is None:
                start_response("400 Bad Request", [("Content-Type", "text/plain")])
                return [b"ReviewBody is required"]
            
            # location has to be a valid location
            if request_params['Location'][0] not in self.valid_locations:
                start_response("400 Bad Request", [("Content-Type", "text/plain")])
                return [b"Location is not valid"]

            # add a unique identifier to the review and timestamp
            request_params['Location'] = request_params['Location'][0]
            request_params['ReviewBody'] = request_params['ReviewBody'][0]
            request_params['ReviewId'] = str(uuid.uuid4())
            request_params['Timestamp'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

            # return response which is the same request params
            response_body = json.dumps(request_params).encode("utf-8")
